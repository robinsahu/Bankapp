...
const createUsernames = function(accs) {
    for (const acc of accs)acc.username = acc.owner.toLowerCase().split(' ').map(function(word) {
        return word[0];
    }).join('');
};
...
const transferValidation = function(receiverAcc, amount) {
    ...
    } else if (currentAccount.username === receiverAcc.username) alert(`You can't transfer money to your own account`);
};
//Transfer to account
...
btnClose.addEventListener('click', function(e) {
    ...
    if (inputCloseUsername.value === currentAccount.username && currentAccount?.pin === Number(inputClosePin.value)) {
        ...
        inputClosePin.value = '';
    } else if (currentAccount.username !== inputCloseUsername.value) alert(`Username is incorrect`);
